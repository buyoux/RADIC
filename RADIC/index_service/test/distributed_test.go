...
var (
	...
	workers []*indexservice.IndexServiceWorker
)

func StartWorkers() {
	workers = make([]*indexservice.IndexServiceWorker, 0, len(workPorts))
	for i, port := range workPorts {
		...
		service.Init(50000, kvdb.BADGER, util.ProjectRootPath+"data/local_db/book_badger_"+strconv.Itoa(i))
...
func TestIndexCluster(t *testing.T) {
	...
	sentinel := indexservice.NewSentinel(etcdServers)
	//测试Add接口
	book := Book{
		ISBN:    "436246383",
		Title:   "上下五千年",
