...
func StartService() {
	...
	service.Init(50000, kvdb.BADGER, util.ProjectRootPath+"data/local_db/book_badger") //不进行服务注册，client直连server
...
func TestIndexService(t *testing.T) {
	...
	//创建client
	client := indexservice.NewIndexServiceClient(conn)

	//测试Search接口
	query := types.NewTermQuery("content", "文物")
	query = query.And(types.NewTermQuery("content", "唐朝"))
	request := &indexservice.SearchRequest{
		Query: query,
	}
	...
	if err != nil {
		...
		//测试Delete接口
		if len(docId) > 0 {
			affect, err := client.DeleteDoc(context.Background(), &indexservice.DocId{DocId: docId})
			if err != nil {
				fmt.Println(err)
				t.Fail()
			} else {
