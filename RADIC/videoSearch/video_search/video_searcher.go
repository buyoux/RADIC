...
import (
	...
	"RADIC/videoSearch/video_search/filter"
	"RADIC/videoSearch/video_search/recaller"
	"reflect"
	"sync"
	"time"
...
type Recaller interface {
	Recall(*common.VideoSearchContext) []*videoSearch.BiliVideo
}
...
func (searcher *VideoSearcher) Recall(searchContext *common.VideoSearchContext) {
	...
	// 并行执行多路召回
	collection := make(chan *videoSearch.BiliVideo, 1000)
	wg := sync.WaitGroup{}
	...
	// 通过map合并多路召回的结果
	videoMap := make(map[string]*videoSearch.BiliVideo, 1000)
...
// 超类定义了一个算法的框架，在子类中重写特定的算法步骤（即recall和filter这两步）
func (searcher *VideoSearcher) Search(searchContext *common.VideoSearchContext) []*videoSearch.BiliVideo {
	t1 := time.Now()
	// 召回
...
func NewAllVideoSearch() *AllVideoSearcher {
...
func NewUpVideoSearch() *UpVideoSearcher {
